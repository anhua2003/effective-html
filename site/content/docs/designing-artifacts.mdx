...
Effective HTML's `design-artifact` skill was derived from Anthropic's official
[`web-artifacts-builder`](https://github.com/anthropics/skills/blob/main/skills/web-artifacts-builder/SKILL.md)
skill for Claude Code. Anthropic's skill combines design guidance with a React
artifact toolchain. This adaptation keeps the creative-direction guidance and
makes it usable with any Effective HTML artifact.
...
## What changed from Claude Code's artifact skill

Anthropic's skill provides a toolchain for elaborate React artifacts. Effective
HTML keeps the parts that help an agent choose a visual premise, palette, type,
composition, and level of treatment. It removes the requirement to use that
toolchain, so the guidance can also shape quiet plans, wireframes, diagrams,
mockups, and standalone documents. This adaptation is maintained by Plannotator
and is not an Anthropic skill.
